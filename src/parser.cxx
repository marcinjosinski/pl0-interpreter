...
// Rozbic to na 2 pliki parser.cxx i interpreter.cxx
// Naprawic dzisiaj call_statement

using namespace pl0;

class parser {
public:
  explicit parser(std::unique_ptr<lexer::lexer> lex)
      : lexer_{std::move(lex)}, top_{std::make_shared<scope::scope>(
                                    "Global Scope")} {
  ...
  ast::statement_ptr procedure_declaration();
  ast::statement_ptr constant_var_declaration();
  ast::statement_ptr variable_declaration();

  ast::statement_ptr statement();

  ast::statement_ptr begin_end_statement();
  ast::statement_ptr while_statement();
  ast::statement_ptr if_statement();
  ast::statement_ptr call_statement();
  ast::statement_ptr read_statement();
  ast::statement_ptr write_statement();
  ast::statement_ptr assign_statement();

  ast::expression_ptr condition();
  ast::expression_ptr expression();
  ast::expression_ptr term();
  ast::expression_ptr factor();
...
// const idet = number, ident2 = number;
// var ident, ident2, ident3;
// procedure ident; const ident3 = lol;

ast::statement_ptr parser::procedure_declaration() {
  ...
  auto proc = std::make_unique<scope::procedure_symbol>(id, top_->get_level());

  top_->define(std::move(proc));

  lexer_->match(token_type::semicolon);

  // fix -> scope name
  top_ = std::make_shared<scope::scope>("Func scope\n", top_);

  std::unique_ptr<ast::block> bl = block();

  top_ = top_->get_enclosing_scope();

  lexer_->match(token_type::semicolon);

  return std::make_unique<ast::procedure_declaration>(std::move(proc),
                                                      std::move(bl));
}

ast::statement_ptr parser::constant_var_declaration() {
  using lexer::token_type;
...
ast::statement_ptr parser::variable_declaration() {
  ...
  for (;;) {
    ...
    auto var = std::make_unique<scope::variable_symbol>(id, 0);
...
std::unique_ptr<ast::block> parser::program() {}

std::unique_ptr<ast::block> parser::block() {
  auto constant_variables =
      lexer_->current_token.is(lexer::token_type::constant)
          ? constant_var_declaration()
          : nullptr;
  auto variables = lexer_->current_token.is(lexer::token_type::var)
